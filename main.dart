...
final _bilyonerToken = Platform.environment['BILYONER_TOKEN'] ?? '';
...
Future<void> main() async {
  ...
  if (_sbUrl.isEmpty || _sbKey.isEmpty || _bilyonerToken.isEmpty) { 
    print('❌ Gerekli env değişkenleri (SUPABASE veya BILYONER_TOKEN) eksik'); 
    exit(1); 
  }
...
try {
  final existing = await http.get(
    Uri.parse('$_sbUrl/rest/v1/live_matches?fixture_id=eq.$fid&select=raw_data,status_short'),
    headers: _sbHeaders(),
  ).timeout(const Duration(seconds: 5));
  if (existing.statusCode == 200) {
    final rows = jsonDecode(existing.body) as List;
    if (rows.isNotEmpty) {
      final dbStatus = rows[0]['status_short'] as String? ?? '';
      if (_isFinished(dbStatus)) {
        print('[BLY] 🚫 fid=$fid zaten bitti ($dbStatus), yeniden eklenmedi');
        _addingFids.remove(fid);
        return;
      }
  ...
  try {
    ...
    if (res.statusCode < 300) {
      ...
      _writeCount++;
        ...
        // Yeni eklenen maç doğrudan 1H veya LIVE ise kick-off say ve bildir
      if ((status == '1H' || status == 'LIVE') && !_kickOffSent.contains(fid)) {
        _kickOffSent.add(fid);
        print('[BLY] 🟢 Kick-off tespit edildi (Yeni Eklendi): fid=$fid $htn vs $atn');
        unawaited(_sendKickOffNotification(fid, htn, atn));
      }
...
void _onBilyonerData(String name, Map<String, dynamic>? v) {
  ...
  if (fixture == null) {
    ...
    // ── BURAYA EKLE ──────────────────────────────────────────
// NS → 1H geçişini yakala
final prevStatus = fixture.statusShort;
final newStatus  = _bilyonerPeriodMap[periodType] ?? fixture.statusShort;

if (prevStatus == 'NS' && (newStatus == '1H' || newStatus == 'LIVE')) {
  if (!_kickOffSent.contains(fid)) {
    _kickOffSent.add(fid);
    print('[BLY] 🟢 Kick-off tespit edildi: fid=$fid ${fixture.homeTeam} vs ${fixture.awayTeam}');
    unawaited(_sendKickOffNotification(fid, fixture.homeTeam, fixture.awayTeam));
  }
}

  final ts        = v['ts'] as Map<String, dynamic>?;
  final homeScore = _int(ts?['hs'] ?? v['home']) ?? fixture.homeScore;
  final awayScore = _int(ts?['as'] ?? v['away']) ?? fixture.awayScore;
...
if (timeStr.isNotEmpty) {
  ...
  body: jsonEncode({
    'status_short': status,
    // Eğer Nesine'den besleniyorsa (Guarded) final skoru da lokaldeki kendi hafızasından yazar
    'home_score':   isGuarded ? fixture.homeScore : homeScore,
    'away_score':   isGuarded ? fixture.awayScore : awayScore,
    'updated_at':   DateTime.now().toIso8601String(),
  }),
).ignore();
...
int? _resolveNesine(int bid, Map item) {
  ...
  if (best != null && bestScore >= 0.55) {
    ...
    // nesine_bid DB'ye yazılıyor — acoreapi bunu okuyarak maçı koruma altına alır
    http.patch(
      Uri.parse('$_sbUrl/rest/v1/live_matches?fixture_id=eq.${best.fixtureId}'),
      headers: {..._sbHeaders(), 'Content-Type': 'application/json'},
      body: jsonEncode({'nesine_bid': bid, 'updated_at': DateTime.now().toIso8601String()}),
    ).ignore();
...
Future<void> _cleanStaleMatches() async {
  try {
    ...
    for (final r in (jsonDecode(res.body) as List).cast<Map>()) {
      ...
      final status = r['status_short'] as String? ?? '';
      ...
      // Statüye göre esnek bekleme süresi
      int maxAge = 6; 
      if (status == 'HT') maxAge = 25; // Devre arası için 25 dakika tolerans
      if (status == 'BT') maxAge = 15; // Uzatmalar öncesi ara için 15 dakika tolerans
      
      if (age > maxAge) stale.add(fid);
    }
...
Future<void> _sendKickOffNotification(int fid, String homeTeam, String awayTeam) async {
  print('[NOTIF] 📨 Kick-off bildirimi: $homeTeam vs $awayTeam (fid=$fid)');
}
